...
const OUTBOX_SCHEDULE_CONFIRM_EVENT = "schedule.confirm.sms";
const OUTBOX_ASSIGNMENT_DISPATCH_EVENT = "assignment.dispatch.sms";
...
async function insertTransitionRow(client, params) {
  ...
  return transitionResult.rows[0].id;
}
...
function buildOutboxTransitionId(ticketId, eventType, components) {
...
function buildOutboxIdempotencyKey(ticketId, eventType, stateTransitionId) {
  return `${ticketId}:${eventType}:${stateTransitionId}`;
}

function buildOutboxSmsPayload(params) {
  const { to, eventType, stateTransitionId, payload, ticketId, transitionId } = params;
  ...
  return {
    ...
    message_key: buildOutboxIdempotencyKey(ticketId, eventType, stateTransitionId),
  };
}

async function insertOutboxMessage(client, params) {
  const { ticketId, eventType, stateTransitionId, to, body, transitionId } = params;
  const payload = buildOutboxSmsPayload({
    to,
    eventType,
    stateTransitionId,
    ticketId,
    ...
    payload: {
      body,
      metadata: {
        reason: `${eventType} message generated by dispatch API`,
        state_transition_id: stateTransitionId,
        transition_id: transitionId,
      },
    },
  });
  const idempotencyKey = buildOutboxIdempotencyKey(ticketId, eventType, stateTransitionId);
  ...
  const insertResult = await client.query(
    ...
    `,
    ["ticket", ticketId, eventType, payload, idempotencyKey],
  );
...
async function confirmScheduleMutation(client, context) {
  ...
  const ticket = update.rows[0];
  const transitionId = buildOutboxTransitionId(ticketId, OUTBOX_SCHEDULE_CONFIRM_EVENT, {
    from_state: existing.state,
    to_state: "SCHEDULED",
    scheduled_start: start,
    scheduled_end: end,
  });
  const recipient = typeof ticket.customer_phone === "string" ? ticket.customer_phone.trim() : null;
  const outboxBody = `Ticket ${ticketId} has been confirmed for ${start} to ${end}.`;
  ...
  const outboxTransitionRow = await insertOutboxMessage(client, {
    ticketId,
    eventType: OUTBOX_SCHEDULE_CONFIRM_EVENT,
    stateTransitionId: transitionId,
    transitionId: auditEventId,
    to: recipient || "UNKNOWN_RECIPIENT",
    body: outboxBody,
  });
  await insertTransitionRow(client, {
    ticketId,
  ...
  if (!outboxTransitionRow?.id) {
    throw new HttpError(
      500,
      "OUTBOX_INSERT_FAILED",
      "Unable to enqueue schedule.confirm SMS message",
    );
  }
...
async function outboxReplayMutation(client, context) {
  const { body, actor, requestId, correlationId, traceId } = context;
  ensureObject(body);
  const outboxId = requireUuidField(body.outbox_id, "outbox_id");

  const existing = await client.query(
    `
      SELECT id, status, aggregate_type, aggregate_id, attempt_count
  ...
  if (existing.rowCount === 0) {
    throw new HttpError(404, "OUTBOX_NOT_FOUND", "Outbox event was not found", {
      outbox_id: outboxId,
      request_id: requestId,
    });
  }
  ...
  if (current.status !== OUTBOX_STATUS_DEAD_LETTER) {
    throw new HttpError(
      409,
      "OUTBOX_REPLAY_INVALID_STATE",
      "Only dead-lettered outbox events can be replayed",
      {
        outbox_id: outboxId,
  ...
  await client.query(
    `
      ...
      WHERE id = $1
    `,
    [outboxId, OUTBOX_STATUS_PENDING],
  );

  await insertAuditEvent(client, {
    ticketId: current.aggregate_type === "ticket" ? current.aggregate_id : null,
    beforeState: null,
    afterState: null,
    actorType: actor.actorType,
    actorId: actor.actorId,
    ...
    payload: {
      ...
      replay: {
        ...
        prior_attempt_count: current.attempt_count,
      },
    },
  });
...
async function dispatchAssignmentMutation(client, context) {
  ...
  const outboxTransitionId = buildOutboxTransitionId(ticketId, OUTBOX_ASSIGNMENT_DISPATCH_EVENT, {
    from_state: existing.state,
    to_state: "DISPATCHED",
    dispatch_mode: dispatchMode ?? null,
    tech_id: body.tech_id,
    provider_id: body.provider_id ?? null,
    requires_ready_to_schedule_stage: requiresEmergencyTriagedReadyStage,
  });
  const recipient =
    typeof existing.customer_phone === "string" ? existing.customer_phone.trim() : null;
  const outboxBody = `Technician ${body.tech_id} assigned for ticket ${ticketId}.`;
  ...
  const outboxMessage = await insertOutboxMessage(client, {
    ticketId,
    eventType: OUTBOX_ASSIGNMENT_DISPATCH_EVENT,
    stateTransitionId: outboxTransitionId,
    transitionId: auditEventId,
    to: recipient || "UNKNOWN_RECIPIENT",
    body: outboxBody,
  });
  if (!outboxMessage?.id) {
    throw new HttpError(
      500,
      "OUTBOX_INSERT_FAILED",
      "Unable to enqueue assignment.dispatch SMS message",
    );
  }
...
function resolveRoute(method, pathname) {
  ...
  if (method === "POST" && pathname === "/outbox/replay") {
    return {
      kind: "command",
      endpoint: "/outbox/replay",
      handler: outboxReplayMutation,
      ticketId: null,
    };
  }
